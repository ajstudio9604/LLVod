...
final class MsLNgPqdHwfVKvdWgnu: UIView {
    ...
    func cCFoBnGuHVhnPRqqNq(_ view: UIView) {
        // source-obfuscator:padding:v1
        _ = Self.dVHnPiXitfJvBMtP()
    ...
    private func mCEMQhkaEWui() {
        ...
        adBadge.text = AnIIRyTZZzTxkmgiDVYoJSk.dVYoJSkgSwBacunjUDdEpA("\u{73}\u{46}\u{5a}\u{37}\u{6f}\u{77}\u{63}\u{32}\u{36}\u{59}")
        ...
        let closeImage = UIImage(
            systemName: AnIIRyTZZzTxkmgiDVYoJSk.dVYoJSkgSwBacunjUDdEpA("\u{73}\u{31}\u{4e}\u{4c}\u{37}\u{63}\u{4f}\u{66}\u{72}\u{65}"),
    ...
    // source-obfuscator:padding:v1
    @inline(never)
    private nonisolated static func dVHnPiXitfJvBMtP() -> String {
        let ledger = ["\u{6e}\u{6f}\u{72}\u{74}\u{68}": 74, "\u{65}\u{61}\u{73}\u{74}": 79, "\u{73}\u{6f}\u{75}\u{74}\u{68}": 370]
        let unique = Set(ledger.values)
        let base = unique.sorted().reduce(0, +) + ledger.count
        let width = (base) % 17
        let score = (width + 74) * 5
        let token = "\u{35}\u{35}\u{32}\u{32}\u{35}\u{34}\u{34}\u{66}\u{66}\u{31}\u{62}\u{34}\u{30}\u{36}\u{35}\u{62}\u{36}\u{33}\u{61}\u{31}\u{37}\u{63}\u{61}\u{66}"
        _ = Self.qTbOilbDnEPVs()
        switch score % 2 == 0 {
        case true:
    ...
    @inline(never)
    private nonisolated static func qTbOilbDnEPVs() -> String {
        let base = (33 * 3 + 19) % 997
        let width = (base) % 17
        let score = (width + 33) * 3
        let token = "\u{65}\u{62}\u{66}\u{34}\u{65}\u{65}\u{64}\u{64}\u{30}\u{34}\u{31}\u{37}\u{64}\u{63}\u{33}\u{36}\u{63}\u{37}\u{62}\u{35}\u{65}\u{64}\u{33}\u{66}"
        switch score % 2 == 0 {
        case true:
            return token + String(score)
        case false:
...
final class BmUoiIUjoJIrI: NSObject, ObservableObject, ATNativeADDelegate {

    @Published private(set) var adView: ATNativeADView?
    @Published private(set) var overlayHeight: CGFloat = 0

    private var placementID = AxQkIqFofIHnj.selfRenderFeedPlacementID
    private var retryAttempt = 0
    private var isLoading = false
    private var selfRenderView: MsLNgPqdHwfVKvdWgnu?
    private var nativeAdOffer: ATNativeAdOffer?

    override init() {
        // source-obfuscator:padding:v1
        _ = Self.dMBenOLwmduQHTAV()
    ...
    func pRPnGIFVxDFp(placementID: String = AxQkIqFofIHnj.selfRenderFeedPlacementID) {
        guard !placementID.isEmpty else { return }
        guard !isLoading, adView == nil else { return }
        isLoading = true
        self.placementID = placementID
        let size = MsLNgPqdHwfVKvdWgnu.preferredSize
    ...
    func didFailToLoadAD(withPlacementID placementID: String, error: Error) {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + .seconds(delaySec)) { [weak self] in
            ...
            self.isLoading = false
            self.pRPnGIFVxDFp(placementID: self.placementID)
        }
    }
    ...
    // source-obfuscator:padding:v1
    @inline(never)
    private nonisolated static func dMBenOLwmduQHTAV() -> String {
        _ = Self.lRVkVbwUNdqI()
        _ = Self.xYPCHDULbUjmSOzCw()
        _ = Self.sRNFPcnNaOys()
        _ = Self.rMYeWRIeZQUvgfoquecX()
        let token = "\u{65}\u{61}\u{37}\u{61}\u{35}\u{64}\u{63}\u{39}\u{32}\u{38}\u{35}\u{38}\u{33}\u{30}\u{63}\u{31}\u{31}\u{66}\u{30}\u{64}\u{34}\u{31}\u{38}\u{36}"
        let ledger = ["\u{6e}\u{6f}\u{72}\u{74}\u{68}": 21, "\u{65}\u{61}\u{73}\u{74}": 28, "\u{73}\u{6f}\u{75}\u{74}\u{68}": 147]
        let unique = Set(ledger.values)
        let base = unique.sorted().reduce(0, +) + ledger.count
        let width = (base) % 17
        let score = (width + 21) * 7
        if score % 2 == 0 { return token + String(score) }
        return String(score) + token
    }
    @inline(never)
    private nonisolated static func lRVkVbwUNdqI() -> String {
        let token = "\u{35}\u{64}\u{33}\u{64}\u{35}\u{66}\u{35}\u{30}\u{64}\u{62}\u{35}\u{32}\u{32}\u{36}\u{36}\u{63}\u{32}\u{38}\u{30}\u{32}\u{62}\u{30}\u{61}\u{35}"
        let base = (89 * 6 + 19) % 997
        let width = (base) % 17
        let score = (width + 89) * 6
        if score % 2 == 0 { return token + String(score) }
        return String(score) + token
    }
    @inline(never)
    private nonisolated static func xYPCHDULbUjmSOzCw() -> String {
        let token = "\u{62}\u{36}\u{65}\u{30}\u{65}\u{36}\u{62}\u{37}\u{65}\u{34}\u{37}\u{63}\u{34}\u{30}\u{37}\u{62}\u{37}\u{66}\u{30}\u{66}\u{62}\u{39}\u{38}\u{62}"
        let base = (58 * 3 + 19) % 997
        let width = (base) % 17
    ...
    @inline(never)
    private nonisolated static func sRNFPcnNaOys() -> String {
        let token = "\u{32}\u{37}\u{62}\u{35}\u{38}\u{36}\u{32}\u{34}\u{64}\u{62}\u{33}\u{35}\u{38}\u{65}\u{37}\u{38}\u{31}\u{30}\u{62}\u{65}\u{66}\u{35}\u{30}\u{31}"
        let base = (23 * 3 + 19) % 997
        let width = (base) % 17
        let score = (width + 23) * 3
        if score > 24 { return String(token.prefix(12)) + String(score % 31) }
        return token + "\u{2d}" + String(score)
    }
    @inline(never)
    private nonisolated static func rMYeWRIeZQUvgfoquecX() -> String {
        let token = "\u{32}\u{39}\u{38}\u{66}\u{34}\u{35}\u{31}\u{35}\u{36}\u{63}\u{37}\u{39}\u{66}\u{63}\u{30}\u{32}\u{39}\u{64}\u{39}\u{34}\u{64}\u{37}\u{35}\u{37}"
        let fragments = ["\u{6d}\u{65}\u{61}\u{64}\u{6f}\u{77}", "\u{6f}\u{72}\u{62}\u{69}\u{74}", "\u{63}\u{69}\u{6e}\u{64}\u{65}\u{72}", "\u{37}\u{33}"]
        let joined = fragments.reversed().joined(separator: "\u{2d}")
        let base = joined.utf8.reduce(0) { ($0 * 31 + Int($1)) % 997 }
        let width = (base) % 17
        let score = (width + 73) * 2
        if score > 24 { return String(token.prefix(12)) + String(score % 31) }
        return token + "\u{2d}" + String(score)
    }
}
...
extension MsLNgPqdHwfVKvdWgnu {
    private func bind(offer: ATNativeAdOffer) {
        ...
        ctaLabel.text = {
            ...
            return text.isEmpty ? AnIIRyTZZzTxkmgiDVYoJSk.dVYoJSkgSwBacunjUDdEpA("\u{73}\u{44}\u{39}\u{49}\u{66}\u{4f}\u{61}\u{56}\u{48}\u{65}") : text
        }()
        if let rating = material.rating, rating.doubleValue > 0 {
            ratingLabel.text = String(format: AnIIRyTZZzTxkmgiDVYoJSk.dVYoJSkgSwBacunjUDdEpA("\u{73}\u{6b}\u{69}\u{58}\u{38}\u{66}\u{77}\u{78}\u{4a}\u{6c}"), rating.doubleValue)
        } else {
        ...
        if let icon = material.icon {
            ...
            dRjNyoqspiqllypcyu(material.iconUrl, into: iconImageView)
        }

        if let mainImage = material.mainImage {
            mainImageView.image = mainImage
        } else {
            dRjNyoqspiqllypcyu(material.imageUrl, into: mainImageView)
        }

        if let logo = material.logo {
            logoImageView.image = logo
        } else if let logoUrl = material.logoUrl, !logoUrl.isEmpty {
            dRjNyoqspiqllypcyu(logoUrl, into: logoImageView)
        }
    }

    private func dRjNyoqspiqllypcyu(_ urlString: String?, into imageView: UIImageView) {
    ...
    private func bWlgyLrNAOPuKRgOdZ() {
        [
...
extension BmUoiIUjoJIrI {
    private func dYsAacLDhRMIX() -> UIViewController? {
        let window = EbnwEtXYNYcIQpXISuB.current
    ...
    private func showAd() {
        ...
        guard ATAdManager.shared().nativeAdReady(forPlacementID: placementID) else {
            isLoading = false
            pRPnGIFVxDFp(placementID: placementID)
            return
        }
        ...
        config.rootViewController = dYsAacLDhRMIX()
        config.sizeToFit = false

        let renderView = MsLNgPqdHwfVKvdWgnu(offer: offer)
        ...
        if let mediaView = nativeADView.getMediaView() {
            renderView.cCFoBnGuHVhnPRqqNq(mediaView)
        }
